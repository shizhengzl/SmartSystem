...
namespace WebAppServices.Controllers
{
    ...
    [ApiController]
    public class SQLConfigController : BaseController
    {
        ...
        public SQLConfigController(IMapper mapper
        ...
        [HttpPost("GetResult")]
        [Authorize]
        public ResponseListDto<SQLConfig> GetResult([FromBody] BaseRequest<SQLConfig> request)
        {
            ResponseListDto<SQLConfig> response = new ResponseListDto<SQLConfig>();
            var data = _appSystemServices.GetEntitys<SQLConfig>(); 
            if (!request.IsNull())
            {
                if (!string.IsNullOrEmpty(request.Filter.ToStringExtension()))
                {
                    data = data.Where(x => x.GetColumnSQL.Contains(request.Filter));
                }
            ...
            response.Data = data.Page(request.PageIndex, request.PageSize).ToList<SQLConfig>();
            return response;
        }
