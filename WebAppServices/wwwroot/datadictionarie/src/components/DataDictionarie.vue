        ...
        <el-table-column prop="columnDescription"
                    ...
                    <template slot-scope="scope">
                      <span v-if="scope.row.isSet">
                        <el-input size="mini" placeholder="请输入字段描述" v-model="scope.row.columnDescription">
                        </el-input>
                      </span>
  ...
  export default {
    ...
    methods: {
      ...
      , getdatabase(node, resolve) {
        let owner = this;
        axios
          .get(owner.goableurl +  '/api/DataBaseConnection/GetDataBaseList')
          .then(response => {
            if (node.level === 0) {
      ...
      , getcolumns(table) {
        ...
        axios
          ...
          .then(response => {
           owner.tableData = response.data.data;
          })
          .catch(function (error) { // 请求失败处理
            console.log(error);
          });
